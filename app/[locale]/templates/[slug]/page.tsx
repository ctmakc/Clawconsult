...
export default async function TemplateDetailPage({ params }: Props) {
  ...
  const tDetail = await getTranslations({ locale, namespace: 'templates.detail' })
  ...
  return (
    <>
      <section className="bg-slate-900 py-14">
        <div className="container-site">
          <nav className="flex items-center gap-1.5 text-xs text-slate-400 mb-6">
            ...
            <span>/</span>
            <Link href="/templates" className="hover:text-slate-300">{tDetail('templates')}</Link>
            <span>/</span>
          ...
          <div className="max-w-3xl">
            <div className="flex flex-wrap gap-2 mb-4">
              <Badge variant={template.complexity === 'low' ? 'L1' : template.complexity === 'medium' ? 'L2' : 'L3'}>
                {complexityLabel[template.complexity]} {tDetail('complexityLabel')}
              </Badge>
            ...
            <div className="mt-5 flex flex-wrap gap-4 text-sm text-slate-400">
              ...
              <span className="flex items-center gap-1.5">
                <Layers className="h-4 w-4" /> {template.includedSkills.length} {tDetail('skillsCountSuffix')}
              </span>
              <span className="flex items-center gap-1.5 font-semibold text-white">
                {tDetail('fromLabel')} {template.pricingFrom}
              </span>
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Button asChild size="lg" variant="outline-white">
                <Link href="/templates">{tDetail('allBlueprints')}</Link>
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className="container-site py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
            <section>
              <h2 className="text-xl font-bold text-slate-900 mb-4">{tDetail('goalsTitle')}</h2>
            ...
            <section>
              <h2 className="text-xl font-bold text-slate-900 mb-5">{tDetail('workflowStagesTitle')}</h2>
            ...
            <section>
              <h2 className="text-xl font-bold text-slate-900 mb-4">{tDetail('includedSkillsTitle')}</h2>
                    ...
                    <div key={item.skillSlug} className="flex items-start gap-3 p-4 rounded-lg border border-slate-200 bg-white">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-sm text-slate-900">{skill?.name ?? item.skillSlug}</span>
                          {skill && (
                      ...
                      <Link href={`/skills/${item.skillSlug}`} className="text-xs text-blue-600 hover:text-blue-700 shrink-0">
                        {tDetail('viewSkill')} <ArrowRight className="inline h-3 w-3" />
                      </Link>
                    </div>
                  )
                })}
            ...
            <section>
              <h2 className="text-xl font-bold text-slate-900 mb-4">{tDetail('successMetricsTitle')}</h2>
            ...
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6">
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium text-slate-900 capitalize">{complexityLabel[template.complexity]}</dd>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <dt className="text-slate-500">{tDetail('skillsLabel')}</dt>
                  <dd className="font-medium text-slate-900">{template.includedSkills.length}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-slate-500">{tDetail('startingFromLabel')}</dt>
                  <dd className="font-bold text-slate-900">{template.pricingFrom}</dd>
                </div>
              </dl>

              <div className="mt-4 space-y-1 text-xs text-slate-500">
                <p className="font-medium text-slate-600">{tDetail('builtForLabel')}:</p>
      ...
      <CtaSection
        ...
        subtitle={`${tDetail('ctaSubtitlePrefix')} ${template.timeline}. ${tDetail('ctaSubtitleSuffix')}`}
