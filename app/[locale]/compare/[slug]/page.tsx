...
export async function generateStaticParams() {
  ...
  return comparisons.map((r) => ({ slug: r.slug }))
}
...
export default async function CompareDetailPage({ params }: Props) {
  const { locale, slug } = await params
  const resource = await getResource(slug, locale as Locale)
  if (!resource || resource.category !== 'comparison') notFound()
  ...
  const schemas = [
    ...
    breadcrumbSchema([
      ...
      { name: 'Compare', url: absoluteUrl(`/${locale}/compare`) },
      { name: resource.title, url: absoluteUrl(`/${locale}/compare/${slug}`) },
    ]),
  ]
  ...
  return (
    <>
      ...
      <section className="bg-slate-900 py-14">
        <div className="container-site max-w-3xl">
          <nav className="flex items-center gap-1.5 text-xs text-slate-400 mb-6">
            ...
            <span>/</span>
            <Link href="/compare" className="hover:text-slate-300">Compare</Link>
            <span>/</span>
            <span className="text-slate-200 line-clamp-1">{resource.title}</span>
          </nav>

          <Badge variant="category" className="mb-4">Comparison</Badge>
      ...
      <CtaSection
        title="Try ClawHelp Free"
        subtitle="See how AI email automation compares in your own inbox. Setup takes under 10 minutes."
        primaryCta="Get Started"
        primaryHref="/contact"
        secondaryCta="More Comparisons"
